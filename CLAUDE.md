...
## Tooling
- **Package manager:** pnpm 11 (pinned via `packageManager` in `package.json`). `engines.node` says `>=20`, but Astro ^6 itself requires Node >=22.12 to run `dev`/`build` — use the version in `.nvmrc` (22).
